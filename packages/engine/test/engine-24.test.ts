...
describe("engine foundation", () => {
  it.skipIf(!hasRustCoverageToolchain)(
    ...
    async () => {
      ...
      const { metricsPath, planPath, reportPath } = requireRustArtifactPaths(result.artifacts);
...
function requireRustArtifactPaths(artifacts: Awaited<ReturnType<typeof runEngine>>["artifacts"]): {
  metricsPath: string;
  planPath: string;
  reportPath: string;
} {
