...
type TerraformTask = PlannedTask & { stageId: StageId };
...
async function runGenericHclFormatStage(
  task: TerraformTask,
...
async function runGenericHclLintStage(
  task: TerraformTask,
  terraformBinary: string,
  runtime: HashicorpRunnerRuntime,
): Promise<StageResult> {
...
async function runTerraformNativeFormatStage(
  task: TerraformTask,
  terraformBinary: string,
  runtime: HashicorpRunnerRuntime,
): Promise<StageResult> {
...
async function runTerraformValidateStage(
  task: TerraformTask,
  runtime: HashicorpRunnerRuntime,
): Promise<StageResult> {
  const files = filterTerraformFiles(task.files);
