...
function createPowerShellAnalyzerDiagnostic(
  ...
  const diagnostic: Diagnostic = {
    ...
    severity: normalizeSeverity(readPowerShellAnalyzerSeverity(record)),
    source: "psscriptanalyzer",
  };
  addPowerShellAnalyzerCode(diagnostic, record);
  addPowerShellAnalyzerRange(diagnostic, record);
  return diagnostic;
}
...
function readPowerShellAnalyzerSeverity(record: Record<string, unknown>): string | undefined {
  const severity = readString(record, "Severity")?.toLowerCase();
  return severity === "parseerror" ? "error" : severity;
}

function addPowerShellAnalyzerCode(diagnostic: Diagnostic, record: Record<string, unknown>): void {
...
function addPowerShellAnalyzerRange(diagnostic: Diagnostic, record: Record<string, unknown>): void {
  const startLine =
