import { supportedStageToolIds } from "./defaults.js";
import type {
  AiqConfigFile,
  AiqInputsConfig,
  AiqLanguageId,
...
function parseSurfaceConfigOverrides(
  ...
  for (const [surfaceId, rawSurfaceConfig] of Object.entries(record)) {
    ...
    const parsedConfig = parseSurfaceConfig(surfaceConfig, `${source}.${surfaceId}`);
...
function parseSurfaceConfig(
  surfaceConfig: Record<string, unknown>,
  source: string,
): Partial<AiqSurfaceConfig> {
  return {
    ...(surfaceConfig.cadenceMs === undefined
      ? {}
      : { cadenceMs: parsePositiveInteger(surfaceConfig.cadenceMs, `${source}.cadenceMs`) }),
    ...(surfaceConfig.cadenceStages === undefined
      ? {}
      : { cadenceStages: parseStageList(surfaceConfig.cadenceStages, `${source}.cadenceStages`) }),
    ...(surfaceConfig.changedOnly === undefined
      ? {}
      : { changedOnly: parseBoolean(surfaceConfig.changedOnly, `${source}.changedOnly`) }),
    ...(surfaceConfig.stages === undefined
      ? {}
      : { stages: parseStageList(surfaceConfig.stages, `${source}.stages`) }),
    ...(surfaceConfig.profile === undefined
      ? {}
      : { profile: parseProfileName(surfaceConfig.profile, `${source}.profile`) }),
    ...(surfaceConfig.publishDiagnostics === undefined
      ? {}
      : {
          publishDiagnostics: parseBoolean(
...
function parseProfileName(value: unknown, source: string): AiqProfileName {
  if (!aiqProfileNames.includes(value as AiqProfileName)) {
    throw new Error(`${source} must be one of ${aiqProfileNames.join(", ")}.`);
  }
