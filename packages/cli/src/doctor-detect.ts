...
import {
  doctorLanguageLabels,
  doctorLanguageOrder,
  doctorMaxScannedFiles,
} from "./doctor-types.js";
import { defaultProjectScopeIgnoredDirectoryNames } from "./project-scope.js";

const markerLanguageByFileName: ReadonlyMap<string, LanguageId> = new Map([
  ["Cargo.toml", "rust"],
  ["go.mod", "go"],
...
const languageByExtension: ReadonlyMap<string, LanguageId> = new Map([
  [".bash", "bash"],
  [".bats", "bash"],
  [".sh", "bash"],
  [".cjs", "javascript"],
...
function addMarkerLanguages(fileName: string, languages: Set<LanguageId>): void {
  const language = markerLanguageByFileName.get(fileName);
