...
#
# Git-based Issues Cache Script (Codex-friendly)
#
# Creates a lightweight issue cache using only git commands and local repo data.
# This works for Codex agents who have git access but no GitHub CLI authentication.
#
# Provides:
...
echo "🗂️  Git-based Issues Cache - Creating offline context for Codex"
...
# Analyze agents.md for Codex guidance
echo "🤖 Extracting Codex guidance from agents.md..."
if [ -f "agents.md" ]; then
	cat >"${CACHE_DIR}/codex-guidance.md" <<EOF
...
EOF
	...
	grep -A 10 -B 2 "Completed Features\|Current Priority\|Critical Path" agents.md >>"${CACHE_DIR}/codex-guidance.md" 2>/dev/null || echo "No system capabilities found in agents.md" >>"${CACHE_DIR}/codex-guidance.md"
fi
...
# Git-based Project Cache for Codex
...
- \`codex-guidance.md\` - Development guidance from agents.md
...
3. **Find development guidance:** \`cat .issue-cache/codex-guidance.md\`
...
if [ "$GH_CACHE_EXISTS" = true ]; then
	...
	echo "   📄 Development guidance for Codex"
	echo "   📄 File change analysis"
	echo "   📄 Project structure overview"
fi
echo ""
echo "🤖 Codex agents can now work with comprehensive offline context"
